...
public class CompetitorService {
    ...
    public Response getProductos() {
        ...
        List<Competitor> competidores = q.getResultList();

        List<Producto> productos = new ArrayList();
        
        for (Competitor competidor : competidores) {
            productos.add(competidor.getProducto());
        }
    ...
    @Path("/namesForA")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getCompetitorNamesStartingWithA() {
    TypedQuery<String> query = (TypedQuery<String>) 
        entityManager.createQuery("SELECT c.name FROM Competitor c WHERE c.name LIKE 'A%'");
    
    List<String> competitorNamesForA = query.getResultList();
    
    return Response.status(200).header("Access-Control-Allow-Origin", "*").entity(competitorNamesForA).build();
    }
